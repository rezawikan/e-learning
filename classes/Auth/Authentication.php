...
class Authentication
{
	...
	public function is_logged_in(){
		if (Session::exists('student') && isset($_COOKIE['student']))
		{
				return true;
		} else {
				return false;
	...
	{
		Session::destroy();
		Session::empty('student');
		setcookie("student", "", time() - (86400 * 30), "/");
	}
	...
	// 			$result = $user->select('adminID, status')
	// 			->where('adminID','=',$id)
	// 			->where('code','=',$code)
	// 			->first();
	//
	...
	// 			$result = $user->select('adminID, status, username, code')->where('email','=',$email)->first();
	//
	// 		if ($result != false) {
	...
	// 			$user = $this->conn;
	// 			$user->setTable('administrator');
	// 			$result = $user->select()->where('adminID','=',$id)->where('tempCode','=',$code)->first();
	//
	// 			if ($result != false) {
	// 				 return true;
	// 			} else {
	...
	// 			$user = $this->conn;
	// 			$user->setTable('administrator');
	// 			$result = $user->select()->where('adminID','=',$id)->where('code','=',$code)->first();
	//
	// 			if ($result != false ) {
	// 					return true;
	// 			} else {
