$(document).ready(function() {
  ...
  $('#form-add-lecturer #datePicker').datepicker({
      format: "mm/dd/yyyy"
  })
  .on('changeDate', function(e) {
        // Revalidate the date field
        $('#form-add-lecturer').formValidation('revalidateField', 'date');
  });
  ...
  .on('changeDate', function(e) {
    ...
    var l = Ladda.create(document.querySelector('button[name=btn-add-lecturer]'));
    //
    $('#form-add-lecturer').formValidation({
      framework: 'bootstrap',
      fields: {
          email: {
              verbose: false, validators: {
    ...
    .on('success.form.fv', function(e) {
        ...
        $.ajax({
          url: 'function/Lecturer/createDataLecturer.php',
          type: 'POST',
          data: $form.serialize(),
          success: function(result){
            console.log(result);
            ...
            $form.formValidation('resetForm', true);
            loadDataLecturer();
            $('#form-add-lecturer').children().removeClass('has-success'); // remove class has-success
            $('#form-add-lecturer')[0].reset(); // reset all fields
            $('button').removeAttr('disabled'); // remove atrribute disabled
            $('button').removeClass('disabled'); // remove class disabled
